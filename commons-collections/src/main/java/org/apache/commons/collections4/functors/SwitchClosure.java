...
public class SwitchClosure<E> implements Closure<E>, Serializable {
     ...
     */
    @Override
    public void execute(final E input) { for (int i = 0; i < iPredicates.length; i += 2) { if (iPredicates[i].evaluate(input)) { iClosures[i].execute(input); return; } } iDefault.execute(input); }
