...
public final class SparseBloomFilter implements BloomFilter {
    ...
    @Override
    public boolean merge(final IndexProducer indexProducer) { Objects.requireNonNull(indexProducer); indexProducer.forEachIndex(this::add); if (!this.indices.isEmpty()) { if (this.indices.last() >= shape.getNumberOfBits()) { throw new IllegalArgumentException(String.format("Value in list %s is greater than maximum value (%s)", this.indices.last(), shape.getNumberOfBits() - 1)); } if (this.indices.first() < 0) { throw new NullPointerException( String.format("Value in list %s is less than 0", this.indices.first())); } } return true; }
