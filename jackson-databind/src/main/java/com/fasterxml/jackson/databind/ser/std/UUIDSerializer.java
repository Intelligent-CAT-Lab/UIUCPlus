    ...
    implements ContextualSerializer // since 2.11.3 (for databind#2815)
{
    ...
    @Override
    public JsonSerializer<?> createContextual(SerializerProvider serializers, BeanProperty property) throws JsonMappingException { JsonFormat.Value format = findFormatOverrides(serializers, property, handledType()); Boolean asBinary = null; if (format != null) { JsonFormat.Shape shape = format.getShape(); if (shape == JsonFormat.Shape.BINARY) { asBinary = true; } else if (shape == JsonFormat.Shape.STRING) { asBinary = false; } // otherwise leave as `null` meaning about same as NATURAL } if ((null != this._asBinary) && (!Objects.equals(asBinary, _asBinary))) { return new UUIDSerializer(asBinary); } return this; }
